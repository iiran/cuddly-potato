...
#ifndef CUDDLY_POTATO_GITHUB_RESEARCH_H
#define CUDDLY_POTATO_GITHUB_RESEARCH_H
...
namespace iiran {
    ...
    public:
        explicit GitHubResearch(std::string out_path) : RemoteResearch{std::move(out_path)} {}

        std::vector<std::string> get_all_files() override {
            std::pair<std::string, std::string> orgAndRepo{std::move(get_org_and_repo(m_root_url))};
            m_file_url_link = std::move(list_repo_file(orgAndRepo.first, orgAndRepo.second));
        ...
        std::string get_file_content(const std::string &path) override {
            ...
            const std::pair<std::size_t, std::size_t> pair = boost::beast::detail::base64::decode(arr.data(),
                                                                                                  raw_content.c_str(),
                                                                                                  raw_content.size());
    ...
    private:
        ...
        static std::pair<std::string, std::string> get_org_and_repo(const std::string &url) {
        ...
        list_repo_file(const std::string &org, const std::string &repo, const std::string &base = "") {
            ...
            try {
                list_repo_file_recursive(format_github_request_url(url), file_url_map, base);
        ...
        static void list_repo_file_recursive(const std::string &url, std::map<std::string, std::string> &file_url_link,
            ...
            if (res.is_object()) {
                ...
                if (res.find("documentation_url") != res.end()) {
                    std::string help{std::move(res["documentation_url"].get<std::string>())};
...
#endif //CUDDLY_POTATO_GITHUB_RESEARCH_H
